...
#Add repopulation to material swarm

print swarm.particleGlobalCount
intSwarm = uw.swarm.PICIntegrationSwarm(swarm, particlesPerCell=100, maxDeletions=0, maxSplits=10)
intSwarm.repopulate()
print swarm.particleGlobalCount
...
advector2       = uw.systems.SwarmAdvector( swarm=surfaceSwarm,     velocityField=velocityField, order=1 )
advector3       = uw.systems.SwarmAdvector( swarm=deformationSwarm, velocityField=velocityField, order=1 )
...
plasticStrain.data[:,0] *= gaussian(swarm.particleCoordinates.data[:,1], 0.0, 0.125) 
...
viscosityMap = { materialA: 0.001, materialV:1.0, materialU:10.0 }
...
frictionInf     = fn.misc.constant(0.2)
...
figYieldStress.append( glucifer.objects.Points(swarm, fn.misc.min(yieldStressFn, 10.0) , pointSize=3.0, fn_mask=materialVariable, 
