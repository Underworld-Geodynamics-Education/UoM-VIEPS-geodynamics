...
outputPath = os.path.join(workdir,"PureShear2D-HR/")
...
# Add repopulation to material swarm (if using Q2/dX1 elements) where no swarm is given
# to the Stokes solver
...
deformationPoints = np.array(np.meshgrid(np.linspace(minX+0.01, maxX-0.01, 1000), np.linspace(0.0, 0.8, 250))).T.reshape(-1,2)
...
viscosityA = 0.01
viscosityV = 1.0
viscosityU = 0.1   ## TRYING CHANGING THIS NUMBER
...
## YOU MIGHT WANT TO CHANGE THE CONSTANT !! 
